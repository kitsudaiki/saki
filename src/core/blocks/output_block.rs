...
impl OutputBlock {
    ...
    /// * `Err(SakiError)` if connection failed
    fn connect_output_buffer(&mut self) -> Result<(), SakiError> {
...
impl Block for OutputBlock {
    ...
    /// * `Err(SakiError)` if an error occurs during training
    fn train(
        &mut self,
        _: usize,
    ...
    /// * `Err(SakiError)` if an error occurs during processing
    fn process(
        &mut self,
        cycle_number: u64,
    ) -> Result<Option<Arc<Mutex<FinishCounter>>>, SakiError> {
    ...
    /// * `Err(SakiError)` if an error occurs during backpropagation
    fn backpropagate(
        &mut self,
        cycle_number: u64,
    ) -> Result<Option<Arc<Mutex<FinishCounter>>>, SakiError> {
    ...
    fn finalize_train(&mut self, _: u64) -> Result<(), SakiError> {
        Ok(())
    }
    ...
    /// * `Ok(())` if finalization was successful
    /// * `Err(SakiError)` if an error occurs during finalization
    fn finalize_process(&mut self, _: u64) -> Result<(), SakiError> {
        Ok(())
    }
