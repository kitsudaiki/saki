...
///
/// # Returns
/// * `Result<(), SakiError>` - Ok(()) if finalization succeeds or after all retries,
///   Err(SakiError) if an error occurs during finalization
fn finalize_task(worker_task: &WorkerTask) -> Result<(), SakiError> {
    ...
    println!("Timeout while try to backpropagate");
    Ok(())
}
...
///
/// # Returns
/// * `Result<(), SakiError>` - Ok(()) if processing succeeds, Err(SakiError) if an error occurs
fn process_task(worker_task: &WorkerTask) -> Result<(), SakiError> {
...
impl WorkerThread {
    ...
    pub fn new(thread_id: usize) -> Self {
        ...
        // println!("Create worker-thread on cpu-thread {thread_id}");

        // Spawn the worker thread
        let handle = thread::spawn(move || {
            // println!("Started worker-thread on cpu-thread {thread_id}");

            // Set CPU affinity to pin the thread to the specified core
            let core_id = core_affinity::CoreId { id: thread_id };
            let res = core_affinity::set_for_current(core_id);
            if !res {
                println!("Failed to pin worker-thread to cpu-thread {thread_id}");
            }
            ...
            // println!("Stopped worker-thread on cpu-thread {thread_id}");
        });
