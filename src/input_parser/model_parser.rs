...
/// Parser for Saki model templates using Pest parser combinator framework.
#[derive(Parser)]
#[grammar = "input_parser/model_parser.pest"]
...
/// # Returns
///
/// * `Result<ModelMeta, SakiError>` - The parsed model or an error if parsing fails
///
/// # Errors
///
/// Returns an SakiError if:
/// * The template version is not supported
/// * The input format is invalid
/// * Any required fields are missing or malformed
pub fn parse_model_template(name: &str, source_text: &str) -> Result<ModelMeta, SakiError> {
    let file_pair = ModelParser::parse(Rule::file, source_text)
        .map_err(|e| {
            SakiError::InvalidInput(format!("Failed to parse model-template with error: {e}",))
        })?
        .next()
        .unwrap();
...
/// # Returns
///
/// * `Result<(), SakiError>` - Ok if initialization succeeds, Err if any step fails
fn init_model(parsed_model: &mut ModelMeta) -> Result<(), SakiError> {
...
/// # Returns
///
/// * `Result<Uuid, SakiError>` - The UUID of the found hexagon or an error if not found
fn search_hexagon(
    hexagons_meta: &HashMap<Uuid, HexagonMeta>,
    position: &Position,
    type_name: &str,
) -> Result<Uuid, SakiError> {
...
/// # Returns
///
/// * `Result<(), SakiError>` - Ok if initialization succeeds, Err if any input is invalid
fn initialize_inputs(parsed_model: &mut ModelMeta) -> Result<(), SakiError> {
    for input_meta in parsed_model.inputs.iter_mut() {
        ...
        if let Some(obj) = parsed_model.hexagons.get_mut(&hexagon_uuid) {
            ...
            return Err(SakiError::InternalError(format!(
...
/// # Returns
///
/// * `Result<(), SakiError>` - Ok if initialization succeeds, Err if any output is invalid
fn initialize_outputs(parsed_model: &mut ModelMeta) -> Result<(), SakiError> {
...
///
/// * `Result<(), SakiError>` - Ok if initialization succeeds, Err if any step fails
fn initialize_hexagons(parsed_model: &mut ModelMeta) -> Result<(), SakiError> {
    update_axons(parsed_model)?;
...
/// # Returns
///
/// * `Result<(), SakiError>` - Ok if all axons are valid, Err if any axon connection is invalid
fn update_axons(parsed_model: &mut ModelMeta) -> Result<(), SakiError> {
...
/// # Returns
///
/// * `Result<(), SakiError>` - Ok if the connection is successful or no neighbor exists
fn connect_hexagon(
...
/// # Returns
///
/// * `Result<(), SakiError>` - Ok if all connections are successful
fn connect_all_hexagons(parsed_model: &mut ModelMeta) -> Result<(), SakiError> {
...
/// # Returns
///
/// * `Result<Uuid, SakiError>` - The UUID of the target hexagon or an error if navigation fails
...
///
/// * `Result<(), SakiError>` - Ok if initialization succeeds, Err if any step fails
fn initialize_target_hexagon_list(parsed_model: &mut ModelMeta) -> Result<(), SakiError> {
